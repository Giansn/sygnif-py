...
say "installed. launchers: $BIN_DIR/{sygnif, nexus, sygnif-desk, sygnif-nexus, sygnif-centre, sygnif-commander}"
case ":$PATH:" in
  *":$BIN_DIR:"*) : ;;
...
say "(via the official claude CLI), sets up a pentest workspace at ~/sygnif-pentest,"
say "and offers to install the Kali pentest tools it's missing (nmap, nikto, gobuster,"
say "sqlmap, hydra, dnsutils, ... via your package manager — skippable, needs sudo)."
say "        sygnif"
say "  (skip the tool install with  SYGNIF_PY_INSTALL_TOOLS=0  before first run.)"
say ""
